...
// EliminatedTx provides a mock function with given fields: _a0
func (_m *Client) EliminatedTx(_a0 *types.RequestEliminatedTx) (*types.ResponseEliminatedTx, error) {
	ret := _m.Called(_a0)

	if len(ret) == 0 {
		panic("no return value specified for EliminatedTx")
	}

	var r0 *types.ResponseEliminatedTx
	var r1 error
	if rf, ok := ret.Get(0).(func(*types.RequestEliminatedTx) (*types.ResponseEliminatedTx, error)); ok {
		return rf(_a0)
	}
	if rf, ok := ret.Get(0).(func(*types.RequestEliminatedTx) *types.ResponseEliminatedTx); ok {
		r0 = rf(_a0)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(*types.ResponseEliminatedTx)
		}
	}

	if rf, ok := ret.Get(1).(func(*types.RequestEliminatedTx) error); ok {
		r1 = rf(_a0)
	} else {
		r1 = ret.Error(1)
	}
